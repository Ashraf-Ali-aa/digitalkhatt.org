...
export interface QuranTextServiceLike {
  quranText: string[][];
  getLineInfo(pageIndex: number, lineIndex: number): { lineType: number; lineWidthRatio: number };
}
  ...
  for (let i = 0; i < lineText.length; i++) {
    ...
    if (char === ' ') {
      if (
        ...
        lineTextInfo.spaces.set(i, 1); // SpaceType.Aya
      } else {
        lineTextInfo.simpleSpaceIndexes.push(i);
        lineTextInfo.spaces.set(i, 0); // SpaceType.Simple
      }
      currentWord = {
        text: '',
        startIndex: i + 1,
